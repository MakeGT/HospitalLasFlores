...
urlpatterns = [
    ...
    url(r'^detalleproducto/(?P<pk>\d+)/', login_required(ProductoDetailView.as_view()), name='detalle_producto'),
    url(r'^agregarlote/(?P<pk>\d+)/', login_required(LoteCreateView.as_view()), name='agregar_lote'),
